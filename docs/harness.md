...
This release retains `airs env` for creating, inspecting, selecting and removing environments, and `airs cli` for the bundled product CLI. See [Login from browser to authorized tools](./login.md) for the exact release version, terminal controls and the full SSO-to-ServiceNow workflow. The `mcp` test channel adds the in-session connection manager and diagnostics dashboard. The published **0.1.0-alpha.22.mcp.4** test release adds a native-only MCP storage default for new environments; fresh anonymous registry installations passed isolated acceptance on all three supported platforms. Existing environments keep their configured storage mode. It carries forward mcp.3's precise credential-failure recovery, recovery commands scoped to the selected environment and supported-Node checks before launch. The protected `latest`, `alpha` and `onboarding` tags remain onboarding.4. See the login lesson for the current installation instructions.
