...
This release retains `airs env` for creating, inspecting, selecting and removing environments, and `airs cli` for the bundled product CLI. See [Login from browser to authorized tools](./login.md) for the exact release version, terminal controls and the full SSO-to-ServiceNow workflow. The `mcp` test channel adds the in-session connection manager and diagnostics dashboard. The published **0.1.0-alpha.22.mcp.4** test release introduced a native-only MCP storage default for new environments; its fresh anonymous registry installations passed isolated acceptance on all three supported platforms. The published **0.1.0-alpha.22.mcp.5** release includes the corrected Ubuntu SSH preparation helper; fresh anonymous installations passed on all three supported platforms, and the bundled helper passed its isolated Ubuntu regression with the default mcp.5 version. Existing environments keep their configured storage mode. It carries forward mcp.3's precise credential-failure recovery, recovery commands scoped to the selected environment and supported-Node checks before launch. The protected `latest`, `alpha` and `onboarding` tags remain onboarding.4. See the login lesson for the current installation instructions.

The published **0.1.0-alpha.22.mcp.6** release improves MCP sign-in: local desktop browser opening, explicit SSH callback instructions, scrollable links and separate authorization, credential-storage and tool-discovery progress. Exact native package acceptance and fresh registry installs passed on all three platforms. The owner’s real Apple Silicon browser sign-in test remains pending.
