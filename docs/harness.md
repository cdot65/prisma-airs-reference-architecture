...
This release retains `airs env` for creating, inspecting, selecting and removing environments, and `airs cli` for the bundled product CLI. See [Login from browser to authorized tools](./login.md) for the exact release version, terminal controls and the full SSO-to-ServiceNow workflow. The `mcp` test channel adds the in-session connection manager and diagnostics dashboard. The published **0.1.0-alpha.22.mcp.3** test release adds precise credential-failure recovery, recovery commands scoped to the selected environment and supported-Node checks before launch. Stable `latest`, `alpha` and `onboarding` remain onboarding.4. See the login lesson for the current installation instructions.
...
Token bundles live in the operating system credential store, while configuration records contain nonsecret settings and binding metadata. The implementation uses Linux Secret Service and macOS Keychain; Windows is outside this release's distribution and end-to-end acceptance scope. Require native MCP storage once by setting top-level `mcp_oauth_credentials_store = "keyring"` in the selected environment's configuration before login. The manager respects this setting but does not enforce it automatically; upstream `auto` mode can fall back to a credentials file. The two platforms have to be verified separately: a successful Linux login does not establish macOS desktop behavior.
