...
After the MCP server returns a safe configuration summary, the harness can include that result in the next model request. That next request travels through the inference gateway. MCP protocol traffic must also traverse the gateway MCP listener. That routing requirement alone does not prove that any particular scanner runs on each MCP message. Verify configured MCP checks and their execution separately. Gateway authorization, upstream object policy, output projection and content scanning remain distinct controls.
