...
**Implemented** describes the reviewed source. **Observed deployment** describes a dated live check. **Release acceptance** applies only to the exact installed package and its recorded lifecycle tests. The gateway and upstream proxy configuration is live; alpha.14 is published under an owner-authorized internal alpha release scope, with hourly frontend refresh explicitly unverified. [Implementation status and public sources](./evidence.md) records the current distinction and supersedes the earlier direct-route acceptance claims.
