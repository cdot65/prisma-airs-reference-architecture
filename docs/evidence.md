...
Test release **0.1.3-alpha.2.mcp.1** is published under the `mcp` tag; stable `latest` remains **0.1.2**. Implemented slices cover attempt-owned cancellation, unavailable-conversation recovery and reports, configuration-bound access checks, time-stamped observations, bounded catalog diagnostics and terminal inference-denial handling, including explicit policy denials returned under HTTP 200. The managed CLI stays at 7.1.5.

Exact candidate and fresh anonymous registry installations passed on native Linux x64, native Linux ARM64 and signed/notarized Apple Silicon. The checks include upgrade/rollback from 0.1.2, native credential reuse, Jev through the actual agent approval path, explicit doctor probes and one-request-only denial handling. Linux x64 acceptance used Alpine; this is not a new Ubuntu or real-account acceptance claim. Apple accepted notarization. Controlled startup and cancellation measurements met the preview budgets; they used a warm OS cache, not a cold-boot benchmark.

The final runtime's full Rust workspace run recorded **18,405 passed, 3 failed and 34 skipped**. The three failures are the individually reviewed experimental remote-shell cases already present in the 0.1.2 baseline; the suite is not reported as fully green. The first preview was superseded before publication after an HTTP 200 retry gap was found. A later Mac test-tooling correction waits for MCP startup before selecting its menu; the original failed CI result is retained separately from subsequent passing installed-package checks.

These are controlled fixtures, not new company SSO, ServiceNow consent/read, paid Jev or model-accuracy acceptance. Real-account acceptance remains required before stable promotion.
